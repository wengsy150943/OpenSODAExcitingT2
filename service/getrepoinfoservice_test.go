...
func TestGetCertainMetric(t *testing.T) {
	...
	a := GetRepoInfoOfMetric("X-lab2017/open-perf", "issue_resolution_duration")
	println(a.Data)
...
func TestGetCertainRepoSpecial(t *testing.T) {
	avg := 11.09
	month := "2020-08"
	metric := "issue_resolution_duration"
	a := GetCertainRepoInfo("X-lab2017/open-digger", metric, month)
	if a.Data["issue_resolution_duration"][month].(map[string]interface{})["avg"] != avg {
